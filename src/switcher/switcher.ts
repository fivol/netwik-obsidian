...
export function createNetwikSwitcher(app: App, plugin: SwitcherPlusPlugin): SwitcherPlus {
    const systemSwitcher = getSystemSwitcherInstance(app)
        ?.QuickSwitcherModal as SystemSwitcherConstructor;

    if (!systemSwitcher) {
        console.log(
            'Netwik++: unable to extend system switcher',
        );
        return null;
    }

    const netwikSwitcherClass = class extends systemSwitcher implements SwitcherPlus {
    ...
    return new netwikSwitcherClass(app, plugin);
}